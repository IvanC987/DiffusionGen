...
class DiffusionModel(nn.Module):
    ...
    def generate(self,
        ...
        assert len(latents.shape) == 4, f"Expected latents tensor to be of shape (b, c, h, w) instead got {latents.shape=}"
        num_images = latents.shape[0]  # Number of images (i.e. batch size)


        # Denoising loop
        denoising_steps = torch.linspace(0, t, num_steps)
        for t_idx in range(len(denoising_steps)-1, -1, -1):  # Iterate in reverse index

            if realtime_denoise:
                # Latents should be of shape (1, c, h, w) if realtime_denoise is set to True
                temp = vae.decode(latents / 0.18215).sample  # Decode from latent space
                # Rescale images to [0, 1] range
                temp = (temp.clamp(-1, 1) + 1) / 2.0

                # Permute into (w, h, c), the shape PIL.Image expects, scale to [0, 255] and clip if needed
                temp = torch.clip(temp[0].permute(2, 1, 0) * 255, min=0, max=255)

                # Then, convert into uint8 dtype and into np arr
                temp = temp.to(torch.uint8).numpy()

                # Finally, save it into designated folder where it should be created beforehand in app.py
                # Usually don't like hardcoding the path here, but it's fine for this case
                Image.fromarray(temp).save(f"./flask_outputs/denoising_temp/{t_idx}.png")


            t = denoising_steps[t_idx].to(torch.int)
            prev_t = denoising_steps[t_idx-1].to(torch.int)

            # Update progress
            if progress_callback is not None:
                # progress_callback(int(100 * (t + 1) / num_steps))
                progress_callback(int(t))
            ...
            # Amazing. This DDIM portion of the implementation took me forever
            # Had to check multiple sources and for some reason, it all came out bad, until I realized my critical error:
            # I was using literally, the previous timestep, t-1, rather than the interpolated timestep, which is technically tau-1
            # Though both are used, they differ in nuance which I overlooked.
            # Long story short, added 'prev_t' and used that rather than t-1
            if use_ddim:
                print(t)
                print(f"Test 1, DDIM")
                # This is the estimate of the original clean image
                x_hat_0 = (latents - sqrt_one_minus_alpha_bar_t * predicted_noise) / self.sqrt_alpha_bar[t]

                if t > 0:
                    if inpainting_mask is not None:
                        clean_ts = torch.tensor([prev_t] * clean_latents.shape[0], device=device).long()
                        # Apply noise using the forward diffusion method and calculated timesteps
                        noised_clean_latents, _ = self._forward_process(clean_latents, clean_ts)

                        # This now the new latent based on the DDIM formula
                        calc_latents = self.sqrt_alpha_bar[prev_t] * x_hat_0 + self.sqrt_one_minus_alpha_bar[prev_t] * predicted_noise
                        latents = (noised_clean_latents * inpainting_mask) + (calc_latents * (-inpainting_mask + 1))
                    else:
                        latents = self.sqrt_alpha_bar[prev_t] * x_hat_0 + self.sqrt_one_minus_alpha_bar[prev_t] * predicted_noise
                else:
                    if inpainting_mask is not None:
                        # clean_ts = torch.tensor([0] * clean_latents.shape[0], device=device).long()
                        # # Apply noise using the forward diffusion method and calculated timesteps
                        # noised_clean_latents, _ = self._forward_process(clean_latents, clean_ts)

                        calc_latents = x_hat_0
                        latents = (clean_latents * inpainting_mask) + (calc_latents * (-inpainting_mask + 1))
                    else:
                        latents = x_hat_0  # No noise for the last step

            else:
                print(t)
                print(f"Test 2, DDPM")
                # This is the exact equation used in the DDPM paper on Page 4 Algorithm 2 Sampling, step 4
                # Paper can be found at https://arxiv.org/pdf/2006.11239
                # x_(t-1) = 1/torch.sqrt(alpha_t) * (x_t - (1-alpha_t) / (sqrt_one_minus_alpha_bar) * eps_theta(x_t, t))
                # Where mean = x_(t-1), latents = x_t, betas = 1-alpha, predicted_noise = eps_theta(x_t, t)
                # Also, note that the first term in the equation is moved as the last in the implementation below
                ...
                if t > 0:
                    ...
                    if inpainting_mask is not None:
                        ...
                        calc_latents = mean + sigma_t * noise  # Then add the noise for stochasticity
                        latents = (noised_clean_latents * inpainting_mask) + (calc_latents * (-inpainting_mask + 1))
                    else:
    ...
    @torch.no_grad()
    def generate_before_inpaint_mask(self, vae: AutoencoderKL, latents: torch.Tensor, prompt_tensor: torch.Tensor, t: int, num_steps: int, cfg_scale: float, use_ddim: bool, device: str, realtime_denoise=False, progress_callback=None):
        self.eval()
        ...
        for t_idx in range(len(denoising_steps)-1, -1, -1):  # Iterate in reverse index
            ...
            if use_ddim:
                ...
                if t > 0:
                    noise = torch.randn_like(latents)
                    sigma_t = torch.sqrt(self.betas[t])
                    latents = mean + sigma_t * noise  # Then add the noise for stochasticity
                else:
                    latents = mean  # No noise for the last step
    ...
    @torch.no_grad()
    def generate_original(self, vae: AutoencoderKL, prompt_tensor: torch.Tensor, num_images: int, img_dim: int, num_steps: int, cfg_scale: float, use_ddim: bool, device: str):
        self.eval()
